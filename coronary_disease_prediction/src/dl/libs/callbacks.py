...
class Callback :
    ...
    def on_batch_end(self, batch, logs: dict | None = None) : pass
    def on_epoch_end(self, epoch:int, logs: dict | None = None) : pass
    def on_train_end(self) : pass


class TrainResultsMonitoring(Callback) :
    def __init__(self):
        self.best_train_score = -1.0
        self.best_valid_score = -1.0

    def on_epoch_end(self, epoch:int, logs: dict | None = None) :
        logs = logs or {}

        train_score = logs.get("train_score", -1.0)
        train_loss = logs.get("train_loss", -1.0)
        valid_score = logs.get("valid_score", -1.0)
        valid_loss = logs.get("valid_loss", -1.0)
        metric = logs.get("metric", "acc")
...
class EarlyStopping(Callback) :
    def __init__(self, monitor: str = "valid_loss", patience: int = 4, min_delta: float = 1e-4) :
        """
            ...
            Note :
                The monitored value must be one of these values : train_score, train_loss, valid_score or valid_loss.
        """
...
class BestModelCallback(Callback) :
    def __init__(self, model: torch.nn.Module | None, optimizer = None, batch_size: int | None = None, scheduler = None, history = None, model_ema = None, save_root: str | None = None) :
        self.model = model
    ...
    def save(self, best_model_path, history_path) :
        ...
        # joblib.dump(self.preprocess, preprocess_path)
    ...
    def on_train_end(self) :
        if self.best_model is not None and self.save_root is not None:
            ...
            # preprocess_path = os.path.join(self.save_root, "preprocess.pkl")

            self.save(best_model_path, history_path)

            print("\nNote: best model saved with success!")
...
class CheckpointCallback(Callback) :
    ...
    def on_epoch_end(self, epoch: int, logs: dict | None = None) :
        if logs is not None:
            ...
            if valid_loss is None or self.save_root is None:
                return
            ...
            if (epoch + 1) % 5 == 0 :
                ...
                # preprocess_path = os.path.join(checkpoint_path, "preprocess.pkl")
...
def callbacks_config(optimizer = None, monitor: str="valid_loss", scheduler_info: dict | None = None,
                     early_stopping_params: dict | None = None, use_tensorboard: bool = False) :
    """
    Params :
    ...
    if optimizer is not None and scheduler_info is not None:
        scheduler_type = scheduler_info.get("type", None)
        scheduler_params = {k: v for k, v in scheduler_info.items() if k != "type"}

        if scheduler_type in ("reduce_lr_on_plateau", "multi_step_lr", "exponential_lr", "cosine_lr_scheduler") and scheduler_params :
