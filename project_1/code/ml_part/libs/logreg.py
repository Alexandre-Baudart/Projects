...
class LogisticReg(Base) :
    ...
    def optimal_params_search(self, X, y, metric: str = "acc", **kwargs) -> dict :
        ...
        _, optim_n_jobs = get_used_cpu(optim_cv_used=True)

        def objective(trial) :
            params = {
                'C': trial.suggest_float("C", 1e-3, 2.0),
                "max_iter": trial.suggest_int("max_iter", 500, 2000),
                "intercept_scaling": trial.suggest_float("intercept_scaling", 1e-3, 1.0),
            }

            verbose = kwargs.get("verbose", 1)

            model = LogisticRegression(fit_intercept=True, tol=1e-6, random_state=42, verbose=verbose, **params)
    ...
    def fit(self, X, y, metric:str = "acc", use_cross_validation: bool = False, **kwargs) :
        ...
        _, cv_n_jobs = get_used_cpu(optim_cv_used=use_cross_validation)
        ...
        logreg_params = {
            ...
            "intercept_scaling": self.params.get("intercept_scaling", 1e-3),
            "fit_intercept": self.params.get("fit_intercept", True),
            "warm_start": self.params.get("warm_start", True),
            "tol": self.params.get("tol", 1e-6),
            "verbose": self.params.get("verbose", 0),
            "random_state": 42,
        }

        steps = [("scaler", self.scaler), ("logreg", LogisticRegression(**logreg_params))]
