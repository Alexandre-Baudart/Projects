...
class RandomForest(Base) :
    ...
    def optimal_params_search(self, X, y, metric: str = "acc", **kwargs) -> dict :
        ...
        def objective(trial) :
            ...
            model = RandomForestClassifier(random_state=42, n_jobs=n_jobs, verbose=verbose, **params)
    ...
    def fit(self, X, y, metric: str = "acc", use_cross_validation: bool = False, **kwargs) :
        ...
        steps = [("random_forest", RandomForestClassifier(**rf_params))]

        self.model = Pipeline(steps)

        if use_cross_validation :
            cv_n_splits = kwargs.get("cv_n_splits", 5)
            cv_n_jobs = kwargs.get("cv_n_jobs", -1)
