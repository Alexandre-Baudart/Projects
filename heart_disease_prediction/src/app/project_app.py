...
content = {
    "fr": {
        "title": "Démo : Prédiction de maladie coronaire",
        "subtitle": "Ajuster les paramètres suivants",
        ...
        "bin_values_to_dataset_values": {
            "Non": False,
            "Oui": True,
            "Femme": "Female",
            "Homme": "Male",
        },
        "cat_values_to_dataset_values": {
...
lang_en = st.toggle("FR", value=False)
...
# Title
st.title(f"{current_content["title"]}")

# Subtitle
st.subheader(f"{current_content["subtitle"]}:")

for feat in features_names:

    # Variables numériques

    if feat in dataset_features["num_cols"]:
        metadata = num_features_metadata[feat]

        inputs[feat] = st.slider(
            current_content["features"][feat].capitalize(),
    ...
    elif feat in dataset_features["cat_cols"]:
        option = st.selectbox( current_content["features"][feat].capitalize(), options=current_content["cat_values"][feat], key=f"select_{feat}")

        if feat == "ca":
            inputs[feat] = option
        else: inputs[feat] = current_content["cat_values_to_dataset_values"][option]

    # Variables binaires

    elif feat in dataset_features["binary_cols"]:
        options = ( (["Female", "Male"] if lang == "en" else ["Femme", "Homme"]) if feat == "sex" else (["No", "Yes"] if lang == "en" else ["Non", "Oui"]) )
        option = st.selectbox( current_content["features"][feat].capitalize(), options=options, key=f"select_{feat}" )
        
        inputs[feat] = current_content["bin_values_to_dataset_values"][option]
...
if st.button(current_content["predict_button"]) :
    payload = {
